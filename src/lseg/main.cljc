(ns lseg.main
  "Kotodama WASM entrypoint for the Lseg clean-room actor (L4) — Clojure port.
  ...
  facts (`lseg.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "lseg_ins"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "lseg_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"lseg.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lseg-compat" :tier tier :entities entities} 200])
